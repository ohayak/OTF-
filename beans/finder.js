...
oneAndListMultiSchemasComponent: function (req, cb) {
        var t1 = new Date().getMilliseconds();
        // Input security Controle
        if (typeof req.session === 'undefined' || typeof req.session.controler === 'undefined') {
            error = new Error('req.session undefined');
    ...
    populateCat: function (req, cb) {
        ...
        try {
            ...
            model.popDocuments(_params, function (err, list) {
                logger.debug('Populate Result  :', list);
                logger.debug('req.session : ' , req.session );
		var res = outil.deleteDefault(list, "nom_categorie");
                res.str = JSON.stringify(res);
		logger.debug('List whit out Default :', res);
                return cb(null, {result: res}); //, user:req.session.login_info.user, "state": state, room: _controler.room});
            });
        } catch (err) { // si existe pas alors exception et on l'intègre via mongooseGeneric
            logger.error(err);
        }
    ...
    populateSCat: function (req, cb) {
        ...
        try {
            ...
            model.popDocuments(_params, function (err, list) {
                logger.debug('Populate Result  :', list);
                logger.debug('req.session : ' , req.session );
		var res = outil.deleteDefault(list, "nom_sous_categorie");
                res.str = JSON.stringify(res);
		logger.debug('List whit out Default :', res);
                return cb(null, {result: res}); //, user:req.session.login_info.user, "state": state, room: _controler.room});
            });
